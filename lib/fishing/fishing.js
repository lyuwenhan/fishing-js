...
async function get (isBig, type) {
	...
	let pri = await gr(variate.dataSaver.getLevel, (isBig + 1) * fishAdd[type]);
	if (type == 4 && isBig) {
		await func.clear();
...
async function draw (mi, ma) {
	...
	if (variate.dataSaver.simple || sizeOk1 || sizeOk2) {
		...
		if (!variate.dataSaver.simple) {
			if (sizeOk1) {
				...
				await func.print("当前为 ", nowSize.rows, " 行")
			}
			if (sizeOk2) {
				await func.print("屏幕宽度需至少为51列");
				await func.print("当前为 ", nowSize.cols, " 列")
			}
		}
	} else {
		if (needCl) {
		...
		for (let i = start; i < 15; i++) {
			for (let j = 0; j < 45; j++) {
				...
				if (paint[i][j] == " " && b) {
					await func.printnl("[m", fuColor[lw], fu[lw], "[m")
				} else {
					await func.printnl("[m", color[i][j], paint[i][j], "[m")
				}
			}
			await func.print()
		}
	}
	await func.print("当前状态: ", statuses[nowStatus]);
	await func.print("累计钓鱼数量: ", variate.dataSaver.cnt, " 当前天气: ", ty[weather[1]], wea[weather[0]]);
	if (ma) {
		if (mi) {
	...
	await func.print(variate.dataSaver.simple ? "按 e 键退出简洁模式" : "按 e 键进入简洁模式");
	swp = false
}
...
async function frontFishing (isBig, type) {
	...
	let slip = await func.random(1, 100) <= variate.dataSaver.slip + (weather[0] == 5) * 10;
	if (slip) {
...
async function fishingChoose () {
	let b = await func.random(1, 100) <= variate.dataSaver.bf;
	if (variate.big) {
		b = true;
		variate.big--
	}
...
async function getGan () {
	...
	await func.print("当前鱼竿: ", fishName[variate.dataSaver.gan], "鱼竿");
...
async function aqua () {
	...
	for (let i = 0; i <= 6; i++) {
		await func.print(fishColor[i], fishName[i], "鱼: ", variate.dataSaver.aqFishCnt[i], "只[m")
	}
...
async function makeFood () {
	while (true) {
		...
		s += "7.退出";
		for (let i = 1; i <= 6; i++) {
			await func.print(fishColor[i], fishName[i], "鱼: ", "[m");
			if (fish[i].length) {
				await func.print("    鱼池: ", fish[i].length, " 只")
			}
			if (variate.dataSaver.fish[i][0]) {
				await func.print("    生鱼: ", variate.dataSaver.fish[i][0], " 只")
			}
			if (variate.dataSaver.fish[i][1]) {
				await func.print("    烤鱼: ", variate.dataSaver.fish[i][1], " 只")
			}
...
async function roastFood () {
	...
	while (true) {
		...
		await func.print("制作熟鱼肉: ", fishColor[d], fishName[d], "鱼: ", "[m");
		...
		if (c == "a" || c == "A") {
			...
			for (let i = 0; i < time; i++) {
				for (let j = 0; j < 20; j++) {
					...
					await func.print();
					await func.print(i * variate.dataSaver.roast, "/", a, " 完成");
					await func.sleep(.5)
				}
			}
			await func.clear();
...
async function eatFood () {
	while (true) {
		...
		await func.print("当前饱食度: ", variate.dataSaver.hungry < 10 ? "[31;1m" : variate.dataSaver.hungry < 30 ? "" : variate.dataSaver.hungry < 35 ? "[32m" : "[32;1m", variate.dataSaver.hungry, "[m");
		await func.print("当前数量: ");
		let b = Array(8).fill(false);
		let s = "";
		for (let i = 0; i <= 6; i++) {
		...
		for (let i = 1; i <= 6; i++) {
			await func.print(fishColor[i], fishName[i], "鱼: ", "[m");
			if (variate.dataSaver.fish[i][0]) {
				await func.print("    生鱼: ", variate.dataSaver.fish[i][0], " 只 +", i + 3)
			}
...
async function eatFoodRoast () {
	while (true) {
		...
		await func.print(variate.dataSaver.hungry < 10 ? "[31;1m" : variate.dataSaver.hungry < 30 ? "" : variate.dataSaver.hungry < 35 ? "[32m" : "[32;1m", variate.dataSaver.hungry, "[m");
		await func.print("当前数量: ");
		let b = Array(8).fill(false);
		let s = "";
		for (let i = 0; i <= 6; i++) {
		...
		for (let i = 1; i <= 6; i++) {
			await func.print(fishColor[i], fishName[i], "鱼: [m");
			if (variate.dataSaver.fish[i][1]) {
				await func.print("    烤鱼: ", variate.dataSaver.fish[i][1], " 只 +", i + 7)
			}
			if (!variate.dataSaver.fish[i][0] && !variate.dataSaver.fish[i][1]) {
				await func.print("    暂无[m")
			}
			...
			if (c == "1") {
				...
				while (true) {
					if (variate.dataSaver.cleaningBall) {
						...
						await func.print("当前污染等级: ", variate.dataSaver.cleaningBall);
						...
						await func.print("    购买花费: $20, 当前金币数量: $", variate.dataSaver.money);
						let c = 0;
						while (true) {
							c = await func.getch();
							if (c == "1" || c == "2") {
				...
				for (let i = 0; i <= 6; i++) {
					for (let j = 0; j < fish[i].length; j++) {
						variate.dataSaver.money += Math.floor(await gr() * (1 - .02 * dirty) * await fresh(fish[i][j]))
					}
					fish[i].length = 0
				}
				await func.clear();
